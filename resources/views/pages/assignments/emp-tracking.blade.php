                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-info btn-sm track-location" data-id="{{ $assignment['employee_id'] }}" data-route-id="{{ $assignment['route_id'] }}" data-waypoints="{{ json_encode($assignment['waypoints']) }}">
...
$(document).ready(function() {
    ...
    function calculateAndDisplayRoute(directionsService, directionsRenderer, waypoints) {
        ...
        const origin = waypts[0].location;
        const destination = waypts[waypts.length - 1].location;
        waypts.shift();
        waypts.pop();
    ...
    function fetchAndDisplayClients(routeId) {
        $.get(`/clients/route/${routeId}`, function(data) {
            if (data && data.length > 0) {
                data.forEach(client => {
                    if (client.latitude && client.longitude) {
                        const latLng = new google.maps.LatLng(client.latitude, client.longitude);
                ...
                console.error('No clients found for this route.');
            }
        }).fail(function(err) {
    ...
    function updateEmployeeLocation(employeeId) {
        ...
        $.get(`/employee/${employeeId}/location`, function(data) {
            if (data && data.location) {
                ...
                marker.setTitle('Current Location'); // Update marker title to current location
            } else {
                console.error('Failed to get location data');
                marker.setTitle('Last Known Location'); // Update marker title to last known location
            }
        }).fail(function(err) {
